...
class AzureLoginHelper {
    ...
    login() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const response = yield vscode_1.window.showInformationMessage(deviceLogin.message, copyAndOpen, close);
                if (response === copyAndOpen) {
                    copypaste.copy(deviceLogin.userCode);
                    opn(deviceLogin.verificationUrl);
                }
                const tokenResponse = yield deviceLogin2(deviceLogin);
    ...
    askForLogin() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield vscode_1.window.showInformationMessage(localize('azure-account.loginFirst', "Not logged in, log in first."), login, cancel);
    ...
    updateFilters(configChange = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const tenantIds = filters && filters.reduce((result, filter) => {
            ...
            for (const session of sessions) {
                const client = new azure_arm_resource_1.SubscriptionClient(session.credentials);
                const subscriptionIds = tenantIds && tenantIds[session.tenantId];
                const subscriptions = yield listAll(client.subscriptions, client.subscriptions.list());
