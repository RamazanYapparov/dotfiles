...
class RegistryRootNode extends nodeBase_1.NodeBase {
    ...
    getAzureRegistries() {
        return __awaiter(this, void 0, void 0, function* () {
            const loggedIntoAzure = yield azureAccount.waitForLogin();
                ...
                for (let i = 0; i < subs.length; i++) {
                    ...
                    for (let j = 0; j < registries.length; j++) {
                        if (registries[j].adminUserEnabled && registries[j].sku.tier.includes('Managed')) {
                            ...
                            let node = new azureRegistryNodes_1.AzureRegistryNode(registries[j].loginServer, 'registry', iconPath);
    ...
    getFilteredSubscriptions() {
        return azureAccount.filters.map(filter => {
            return {
